...
if (!in_array($type, ['qemu', 'lxc']) || !in_array($action, ['start', 'stop'])) {
...
$result = $action === 'start' 
    ? startVM($config['proxmox_host'], $config['node'], $type, $vmid, $auth['ticket'], $auth['csrf'])
    : stopVM($config['proxmox_host'], $config['node'], $type, $vmid, $auth['ticket'], $auth['csrf']);
